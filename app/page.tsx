...
import { Search, Shield, Database, Sparkles, ChevronRight, Brain, Layers, BarChart3, Cpu, Activity, Lock, Upload } from 'lucide-react';
        ...
        <div className="container-neo relative z-10">
          ...
          <div className="max-w-4xl mx-auto text-center mb-20">
            ...
            <p className="text-lg md:text-xl text-slate-400 mb-10 max-w-2xl mx-auto leading-relaxed animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
              Empowering security teams with AI-driven insights. Search, analyze, and manage your security knowledge base with unprecedented clarity.
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-24">
            {[
              { title: 'Semantic Search', desc: 'AI-powered deep search', icon: Search, href: '/search', color: 'from-sky-400 to-blue-500' },
              { title: 'Add Knowledge', desc: 'Expand the database', icon: Database, href: '/admin/qa', color: 'from-emerald-400 to-teal-500' },
              { title: 'Smart Match', desc: 'Auto-fill answers', icon: Sparkles, href: '/admin/match-answers', color: 'from-purple-400 to-pink-500' },
              { title: 'Bulk Import', desc: 'Process Excel files', icon: Upload, href: '/admin/import', color: 'from-orange-400 to-amber-500' }
            ].map((card, i) => (
              <Link key={i} href={card.href} className="block group">
                <div className="glass-card h-full p-8 flex flex-col items-start hover:bg-white/[0.02]">
                  ...
                  <h3 className="text-xl font-bold text-slate-100 mb-2 group-hover:text-white transition-colors">{card.title}</h3>
              ...
              {[
                ...
                <div key={i} className="flex flex-col items-center text-center md:items-start md:text-left">
                  ...
                  <div className="text-4xl font-bold text-white tracking-tight">
                    {loading ? <span className="animate-pulse bg-white/10 rounded h-10 w-24 block"></span> : stat.value}
                  </div>
                </div>
              ))}
            </div>
